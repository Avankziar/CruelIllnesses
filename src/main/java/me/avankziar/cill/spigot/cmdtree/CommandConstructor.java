package main.java.me.avankziar.cill.spigot.cmdtree;

import java.util.ArrayList;

import main.java.me.avankziar.cill.spigot.CruelIllnesses;

public class CommandConstructor extends BaseConstructor
{
    public ArrayList<ArgumentConstructor> subcommands;
    public ArrayList<String> tablist;

	public CommandConstructor(String path, boolean canConsoleAccess,
    		ArgumentConstructor...argumentConstructors)
    {
		super(CruelIllnesses.getPlugin().getYamlHandler().getCommands().getString(path+".Name"),
				path,
				CruelIllnesses.getPlugin().getYamlHandler().getCommands().getString(path+".Permission"),
				CruelIllnesses.getPlugin().getYamlHandler().getCommands().getString(path+".Suggestion"),
				CruelIllnesses.getPlugin().getYamlHandler().getCommands().getString(path+".CommandString"),
				CruelIllnesses.getPlugin().getYamlHandler().getCommands().getString(path+".HelpInfo"),
				canConsoleAccess);
        this.subcommands = new ArrayList<>();
        this.tablist = new ArrayList<>();
        for(ArgumentConstructor ac : argumentConstructors)
        {
        	this.subcommands.add(ac);
        	this.tablist.add(ac.getName());
        }
        CruelIllnesses.getPlugin().getCommandTree().add(this);
    }
}