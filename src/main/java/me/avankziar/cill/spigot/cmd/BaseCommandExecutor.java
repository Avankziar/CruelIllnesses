package main.java.me.avankziar.cill.spigot.cmd;
...
import main.java.me.avankziar.cill.general.ChatApi;
import main.java.me.avankziar.cill.spigot.CruelIllnesses;
import main.java.me.avankziar.cill.spigot.assistance.MatchApi;
import main.java.me.avankziar.cill.spigot.cmdtree.ArgumentConstructor;
...
public class BaseCommandExecutor implements CommandExecutor
{
	...
	public BaseCommandExecutor(CruelIllnesses plugin, CommandConstructor cc)
	{
		this.plugin = plugin;
	...
	public boolean onCommand(CommandSender sender, Command cmd, String lable, String[] args) 
	{
		...
		{
			CruelIllnesses.log.info("/%cmd% is only for Player!".replace("%cmd%", cc.getName()));
			return false;
		}
				...
				if(args[i].equalsIgnoreCase(ac.getName()))
				{
					if(length >= ac.minArgsConstructor && length <= ac.maxArgsConstructor)
					{
						if(player.hasPermission(ac.getPermission()))
						{
							...
							{
								CruelIllnesses.log.info("ArgumentModule from ArgumentConstructor %ac% not found! ERROR!"
		...
		player.spigot().sendMessage(ChatApi.clickEvent(plugin.getYamlHandler().getLang().getString("InputIsWrong"),
				ClickEvent.Action.RUN_COMMAND, CruelIllnesses.infoCommand));
		return false;
	}
	...
	public void baseCommands(final Player player, int page)
	{
		...
		player.sendMessage(ChatApi.tl(plugin.getYamlHandler().getLang().getString(
				CruelIllnesses.infoCommandPath+".Headline")));
		...
		pastNextPage(player, CruelIllnesses.infoCommandPath, page, lastpage, CruelIllnesses.infoCommand);
	}
